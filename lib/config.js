 ...
 *   school            { name, short, phone, principal }  ← 다른 학교에서 쓰려면 여기만 바꾸면 됩니다 (규칙·예시의 학교명이 따라 바뀜)
...
const DEFAULTS = {
  ...
  // 훑기: 새 글이 있는 페이지만 넘기며(최대 maxPages 쪽) 제주 모든 초등학교 글을 모아 보관합니다. 본문은 하루 maxBodies 건까지.
  scan: { maxPages: 6, delayMs: 1500, fetchBodies: true, maxBodies: 40, includeKindergarten: false },
  // 초안: 보관함의 최근 다른 학교 글을 참고 예시로 몇 편 덧붙일지 (0 이면 안 붙임)
  draft: { recentExamples: 12 }
};
