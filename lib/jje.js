...
function parseList(html) {
  ...
  $('li.bdGall3').each((i, li) => {
    ...
    if (!sid) return;
    // 이름·전화·날짜가 모두 .bdGall3Name 류의 span 으로 나열되므로 내용을 보고 가른다
    const spans = $(li).find('.NameDate span, .bdGall3Name').map((j, e) => $(e).text().trim()).get().filter(Boolean);
    const date = spans.map(normDate).find(Boolean) || '';
    const phone = spans.find(t => /\d{2,3}\)?[-\s]?\d{3,4}[-\s]?\d{4}/.test(t) && !normDate(t)) || '';
    const school = spans.find(t => !normDate(t) && t !== phone) || '';
    items.push({
      dataSid: sid[1],
...
/** 학교 이름으로 유치원인지 판단 (게시판이 유치원·초등 공용이므로 초등만 고를 때 씀) */
function isKindergarten(school) { return /유치원|어린이집/.test(school || ''); }

/**
 * 새 글만 페이지를 넘기며 모아 옵니다. 이미 본 글(seen)만 있는 페이지를 만나면 멈춥니다.
 * @param {object} cfg
 * @param {(dataSid:string)=>boolean} seen  이미 보관한 글인지
 * @param {{maxPages?:number, fetchText?:Function}} opts
 */
async function fetchNew(cfg, seen, opts = {}) {
  const maxPages = Math.max(1, Math.min(opts.maxPages || cfg.scan.maxPages || 5, 10));
  const get = opts.fetchText || fetchText;
  const fresh = [];
  let total = null, pages = 0;
  for (let p = 1; p <= maxPages; p++) {
    const r = parseList(await get(listUrl(cfg, p), { delayMs: cfg.scan.delayMs }));
    pages = p;
    if (r.total != null) total = r.total;
    if (!r.items.length) break;
    const newOnes = r.items.filter(it => !seen(it.dataSid) && !fresh.some(f => f.dataSid === it.dataSid));
    fresh.push(...newOnes);
    if (newOnes.length === 0) break;                    // 이 페이지는 전부 본 글 → 그 뒤도 본 글
  }
  return { total, pages, items: fresh, truncated: pages >= maxPages && fresh.length >= pages * 10 };
}
...
module.exports = { UA, parseList, parseView, listUrl, searchUrl, fetchList, fetchNew, searchSchool, fetchView, fetchText, normDate, isKindergarten };
